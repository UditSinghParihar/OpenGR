...
void get_source_points(const vector<Point3D>& cloud, const std::array<int, 4>& indexes){
    ...
    for(int i=0; i<indexes.size(); ++i){
        int index = indexes[i];
        std::cout << "Point:\n" << cloud[index].pos() << std::endl;
    }
}

void get_target_points(const vector<Point3D>& cloud, const std::array<int, 4>& indexes){
    std::cout << "Points in target Q:\n";
    for(int i=0; i<indexes.size(); ++i){
        int index = indexes[i];
        std::cout << "Point:\n" << cloud[index].pos() << std::endl;
    }
...
int main(int argc, char **argv) {
  ...
  {
    fprintf(stderr, "Debugging for corresponding points\n--\n");
  }
